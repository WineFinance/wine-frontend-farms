...
const Farms: React.FC<FarmsProps> = (farmsProps) => {
  ...
  const wineOnlyFarms = activeFarms.filter(
    (farm) => farm.lpSymbol.startsWith("WINE"),
  ...
  const stakedPastaOnlyFarms = wineOnlyFarms.filter(
    (farm) => farm.userData && new BigNumber(farm.userData.stakedBalance).isGreaterThan(0),
  )
  ...
  const farmsList = useCallback(
    (farmsToDisplay, removed: boolean) => {
      ...
      const farmsToDisplayWithAPY: FarmWithStakedValue[] = farmsToDisplay.map((farm) => {
        ...
        //   return farm
        // }
               const cakeRewardPerBlock = new BigNumber(farm.winePerBlock || 1).times(new BigNumber(farm.poolWeight)) .div(new BigNumber(10).pow(18))
        const cakeRewardPerYear = cakeRewardPerBlock.times(BLOCKS_PER_YEAR)
        const cakeRewardPerMonth = cakeRewardPerYear.div(12)

        let apy = cakePrice.times(cakeRewardPerYear);
        if (farm.quoteTokenSymbol === QuoteToken.WINE) {
...
// wineOnly={pastOnly} setPastaOnly={setPastaOnly}
    ...
    <Page>
      ...
      <div>
        ...
        <FlexLayout>
          ...
          <Route exact path={`${path}/wine`}>
          {stakedOnly ? farmsList(stakedPastaOnlyFarms, false) : farmsList(wineOnlyFarms, false)}
          </Route>
      ...
      <Image src="/images/wine/8.png" alt="illustration" width={1357} height={300} responsive />
    </Page>
  )
}
